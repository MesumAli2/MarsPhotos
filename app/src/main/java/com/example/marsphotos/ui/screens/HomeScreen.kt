...
fun PhotosGridScreen(photos: List<MarsPhoto>, modifier: Modifier = Modifier) {
    LazyVerticalGrid( columns = GridCells.Adaptive(150.dp),
        ...
        items(items = photos, key = { photo -> photo.id }) {
            photos -> MarsPhotoCard(photo = photos, modifier = modifier
            .padding(4.dp)
            .fillMaxWidth()
            .aspectRatio(1.5f))
        }
...
Image(modifier = modifier,painter = painterResource(id = R.drawable.loading_img), contentDescription = stringResource(id = R.string.loading))
}
